...
export const useBountyContractStore = defineStore('bountyContract', {
  ...
  actions: {
    initialize(contract: BountyContract, bountyId: string, reset = false) {
      return new Promise((resolve, reject) => {
        ...
        if (this.dontContract) {
          services['bounty@bounty-state']({ bountyID: bountyId }).then(response => {
            ...
            if (!error) {
              ...
              this.bountyContractInfo.status = this.bountyContractInfo.status || data.myStatus || 0
        ...
        contract
          ...
          .then(response => {
            ...
            this.bountyContractInfo.myDepositAmount = Number(ethers.utils.formatEther(response[9]))
